...
// edx=int argb
    ...
    int CodePlot(void *code,int depth,int amask,int rmask,int gmask,int bmask)
    {
        ...
        {
            if (amask==0 && depth>24) _and(eax,0xffffff);
        }
        else
        {
            ...
            _and	(eax,bmask);
            shift	(ecx,rmsb-23);
            if (amask)	_and	(edx,amask);			//alph
            _and	(ebx,gmask);
            if (amask)	_or		(eax,edx);				//alph
            _and	(ecx,rmask);
            _or		(eax,ebx);
            _or		(eax,ecx);
        }
...
//			case 24:load16(eax,ecx);shift(eax,-16);load8(eax,ecx,2);break;
        ...
        else
        {
            ...
            _and	(ecx,rmask);
            shift	(ecx,23-rmsb);
            if (amask)	_and	(edx,amask);
            _or		(eax,ebx);
            if (amask)	shift	(edx,31-amsb);
            _or		(eax,ecx);
            if (amask)	_or		(eax,edx);
        }
        pop(ebx);
        int oor=0;
